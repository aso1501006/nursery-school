$(function(){
    ...
    var thumbnailView = function(file,resolver){
        ...
        if(!(type == 'image')){
             /*
             window.alert('画像以外の形式です、動画をアップロードしましょう。');
             formData.delete('file');
             */
        }else{
            cnt++;//ここでカウントする
            ...
            //fileViewに動画追加
            //サムネイル風になる
            $('#fileView').append('<div class="view-contaier"> <div> <img class="file" src="' + blobUrl + '"></div> <span> がぞ～ </span> </div>');
        }
        //アップロード処理
        //uploadFiles(formData);
        resolver(this);
