...
require_once dirname(__FILE__)."/Photos_TagsTblDT.php";
require_once dirname(__FILE__)."/Movies_TagsTblDT.php";
  ...
  public function dbDisconnect(){
    ...
    public function schedule_select($nursery_school_id,$schedule_date){
      try{
            ...
            while($row = $stmt -> fetch(PDO::FETCH_ASSOC)){
                ...
                //DBから取れた情報をカラム毎に、クラスに入れていく
                //$rowData->set_nursery_school_id($row["nursery_school_id"]);
                $rowData->set_schedule_id($row["schedule_id"]);
                $rowData->set_nursery_school_id($row["nursery_school_id"]);
                $rowData->set_schedule_date($row["schedule_date"]);
                $rowData->set_schedule_name($row["schedule_name"]);
                $rowData->set_schedule_content($row["schedule_content"]);
                $rowData->set_tag_id($row["tag_id"]);
    ...
    public function movie_select_new($nursery_school_id,$limit_num){
      try{
            ...
            $this->dbConnect();
            //SQLを生成
            $stmt = $this->myPdo->prepare('SELECT movie_id,nursery_school_id,movie_name,movie_date,movie_path,movie_URL FROM movies WHERE nursery_school_id = :nursery_school_id ORDER BY movie_date DESC limit 10');
            $stmt->bindValue(':nursery_school_id', $nursery_school_id, PDO::PARAM_STR);
    ...
    public function movie_select_tag($tag_id){
      try{
            //DBに接続
            $this->dbConnect();
            //SQLを生成
            $stmt = $this->myPdo->prepare('SELECT movie_id,tag_id FROM movies_tags WHERE tag_id = :tag_id');
            $stmt->bindValue(':tag_id', $tag_id, PDO::PARAM_STR);
    ...
    public function movie_insert($nursery_school_id,$movie_name,$movie_date,$movie_path){
      try{
            //DBに接続
            $this->dbConnect();
            //SQLを生成
    ...
    public function movie_update($movie_id,$movie_name,$movie_date){
      try{
            //DBに接続
            $this->dbConnect();
            //SQLを生成
            $stmt = $this->myPdo->prepare('UPDATE movies SET movie_name = :movie_name movie_date = :movie_date WHERE movie_id = :movie_id');
            $stmt->bindValue(':movie_id', $movie_id, PDO::PARAM_STR);
            $stmt->bindValue(':movie_name', $movie_name, PDO::PARAM_STR);
            $stmt->bindValue(':movie_date', $movie_date, PDO::PARAM_STR);
    ...
    public function movie_update_URL($movie_id,$movie_URL){
      try{
            //DBに接続
            $this->dbConnect();
            //SQLを生成
            $stmt = $this->myPdo->prepare('UPDATE movies SET movie_URL = :movie_URL WHERE movie_id = :movie_id');
            $stmt->bindValue(':movie_id', $movie_id, PDO::PARAM_STR);
            $stmt->bindValue(':movie_name', $movie_URL, PDO::PARAM_STR);
    ...
    public function photo_select_new($nursery_school_id,$limit_num){
      try{
            ...
            $this->dbConnect();
            //SQLを生成
            $stmt = $this->myPdo->prepare('SELECT photo_id,nursery_school_id,photo_name,photo_date,photo_path FROM photos WHERE nursery_school_id = :nursery_school_id ORDER BY photo_date DESC limit 10');
            $stmt->bindValue(':nursery_school_id', $nursery_school_id, PDO::PARAM_STR);
    ...
    public function photo_select_tag($tag_id){
      try{
            //DBに接続
            $this->dbConnect();
            //SQLを生成
            $stmt = $this->myPdo->prepare('SELECT photo_id,tag_id FROM photo_tags WHERE tag_id = :tag_id');
            $stmt->bindValue(':tag_id', $tag_id, PDO::PARAM_STR);
            ...
            while($row = $stmt -> fetch(PDO::FETCH_ASSOC)){
              //データを入れるクラスをnew
              $rowData = new photos_TagsTblDT();

              //DBから取れた情報をカラム毎に、クラスに入れていく
              //$rowData->set_nursery_school_id($row["nursery_school_id"]);
              
                $rowData->photo_id = $row["photo_id"];
                $rowData->tag_id = $row["tag_id"];
    ...
    public function photo_insert($nursery_school_id,$photo_name,$photo_date,$photo_path){
      try{
            //DBに接続
            $this->dbConnect();
            //SQLを生成
    ...
    public function photo_update($photo_id,$photo_name,$photo_date){
      try{
            //DBに接続
            $this->dbConnect();
            //SQLを生成
            $stmt = $this->myPdo->prepare('UPDATE photos SET photo_name = :photo_name photo_date = :photo_date WHERE photo_id = :photo_id');
            $stmt->bindValue(':photo_id', $photo_id, PDO::PARAM_STR);
            $stmt->bindValue(':photo_name', $photo_name, PDO::PARAM_STR);
            $stmt->bindValue(':photo_date', $photo_date, PDO::PARAM_STR);
    ...
    public function movie_tag_delete($movie_id,$tag_id){
      try{
            //DBに接続
            $this->dbConnect();
            //SQLを生成
            $stmt = $this->myPdo->prepare('DELETE FROM movie_tags WHERE movie_id = :movie_id AND tag_id = :tag_id');
            $stmt->bindValue(':movie_id', $movie_id, PDO::PARAM_STR);
            $stmt->bindValue(':tag_id', $tag_id, PDO::PARAM_STR);
    ...
    public function photo_tag_insert($photo_id,$tag_id){
      try{
            ...
            $this->dbConnect();
            //SQLを生成
            $stmt = $this->myPdo->prepare('INSERT INTO movies_tags (photo_id,tag_id) VALUES (:photo_id,:tag_id)');
            $stmt->bindValue(':photo_id', $photo_id, PDO::PARAM_STR);
            $stmt->bindValue(':tag_id', $tag_id, PDO::PARAM_STR);
    ...
    public function photo_tag_delete($photo_id,$tag_id){
      try{
            //DBに接続
            $this->dbConnect();
            //SQLを生成
            $stmt = $this->myPdo->prepare('DELETE FROM photo_tags WHERE photo_id = :photo_id AND tag_id = :tag_id');
            $stmt->bindValue(':photo_id', $photo_id, PDO::PARAM_STR);
            $stmt->bindValue(':tag_id', $tag_id, PDO::PARAM_STR);
    ...
    public function movie_get_tag($movie_id){
      try{
            ...
            $this->dbConnect();
            //SQLを生成
            $stmt = $this->myPdo->prepare('SELECT movie_id,tag_id FROM movies_tags WHERE movie_id = :movie_id');
            $stmt->bindValue(':movie_id', $movie_id, PDO::PARAM_STR);
    ...
    public function photo_get_tag($photo_id){
      try{
            ...
            $this->dbConnect();
            //SQLを生成
            $stmt = $this->myPdo->prepare('SELECT photo_id,tag_id FROM photos_tags WHERE photo_id = :photo_id');
            $stmt->bindValue(':photo_id', $photo_id, PDO::PARAM_STR);
